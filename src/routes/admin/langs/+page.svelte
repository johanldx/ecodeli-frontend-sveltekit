...
{#if $showAddModal}
	<div class="modal modal-open">
		<div class="modal-box">
			<h3 class="text-lg font-bold">Ajouter une langue</h3>
			<input
				type="text"
				class="input input-bordered mt-4 w-full"
				placeholder="ex: es, de, it"
				bind:value={$newLang}
			/>
...
{#if $showDeleteModal}
	<div class="modal modal-open">
		<div class="modal-box">
			<h3 class="text-error text-lg font-bold">Confirmer la suppression</h3>
...
<div class="min-h-screen space-y-6 p-6">
	...
	<div class="flex flex-wrap items-end gap-4">
		<div class="form-control w-full max-w-xs">
			<label class="label" for="lang-select"
				><span class="label-text">Choisir une langue</span></label
			>
			<select
				id="lang-select"
	...
	<JsonEditor
		...
		on:update={(e) => handleJsonUpdate(e.detail)}
	/>
