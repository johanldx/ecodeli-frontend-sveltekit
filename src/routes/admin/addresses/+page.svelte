<script lang="ts">
    ...
    let editedAddress: Address = { id: 0, userId: 0, name: '', address: '', cp: '', city: '', country: '', created_at: '', user: { id: 0, first_name: '', last_name: '', email: '' } };
  ...
  <div class="p-6 min-h-screen">
    <h1 class="text-2xl mt-10 mb-5 font-medium">Adresses</h1>
    ...
    {#if showModalEdit}
      <div class="modal modal-open">
        <div class="modal-box">
          <h2 class="text-xl font-semibold text-center mb-4">Modifier l’adresse</h2>
          <input class="input input-bordered w-full mb-2" bind:value={editedAddress.name} placeholder="Nom" />
          <input class="input input-bordered w-full mb-2" bind:value={editedAddress.address} placeholder="Adresse" />
          <input class="input input-bordered w-full mb-2" bind:value={editedAddress.cp} placeholder="Code postal" />
          <input class="input input-bordered w-full mb-2" bind:value={editedAddress.city} placeholder="Ville" />
          <input class="input input-bordered w-full mb-2" bind:value={editedAddress.country} placeholder="Pays" />
    ...
    {#if showModalDelete}
      <div class="modal modal-open">
        <div class="modal-box">
          <h2 class="text-xl font-semibold text-center mb-4">Supprimer l’adresse</h2>
          {#if selectedAddress}
            <p class="text-center">Confirmez-vous la suppression de l’adresse <strong>{selectedAddress.name}</strong> ?</p>
          {/if}
          <div class="modal-action">
