<script lang="ts">
  ...
  async function loadServiceTypes() {
    ...
    serviceTypes = all;
    const auths = await fetchFromAPI(`/personal-service-type-authorizations?providerId=${providerId}`, {
      method: 'GET',
      headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${get(accessToken)}` },
  ...
  function prevWeek() { currentWeek--; loadSchedules(); }
  function nextWeek() { currentWeek++; loadSchedules(); }
...
<div class="bg-base-100 min-h-screen p-6">
  <div class="mx-auto max-w-7xl">
    ...
    <tr class="hover:bg-base-100">
      ...
      {#each dayKeys as dk}
        <td class="relative border p-0 border-base-300 min-w-32 min-h-16">
          <button
            type="button"
            on:click={() => handleCellClick(dk, slot)}
            class="absolute inset-0 w-full h-full z-10 opacity-0"
            aria-label="Add schedule for {dayLabels[dk as keyof typeof dayLabels]} at {slot}"
          ></button>
          ...
          {#each schedules.filter(s => {
            ...
            <button
              ...
              class="absolute inset-0 z-20 flex items-center justify-center p-1 text-xs font-semibold shadow-sm
                     {sched.status==='available'?'bg-green-500 text-white':'bg-red-500 text-white'}"
            >
  ...
  {#if showAddModal}
    <dialog class="modal modal-open">
      <div class="modal-box">
        <h3 class="font-author text-lg mb-4">{addSlotLabel}</h3>
        <div class="form-control mb-2">
          <label class="label" for="service-type"><span class="label-text">Type de service</span></label>
          <select id="service-type" bind:value={newSchedule.personalServiceTypeId} class="select select-bordered">
        ...
        <div class="grid grid-cols-2 gap-4 mb-2">
          ...
          <div class="form-control">
            ...
            <select id="start-slot" bind:value={newSchedule.startSlot} class="select select-bordered">
        ...
        <div class="modal-action">
          ...
          <button class="btn btn-error" on:click={() => (showAddModal = false)}>{cancelLabel}</button>
        </div>
      </div>
    </dialog>
  {/if}
  ...
  {#if showDetailModal && selectedSchedule}
    <dialog class="modal modal-open">
      <div class="modal-box">
        ...
        <p class="py-2">
          {dayLabels[dayKeys[dayjs(selectedSchedule!.startTime).day()-1 || 6] as keyof typeof dayLabels]}
