<script lang="ts">
	...
	interface Conversation {
		id: number;
		adType: 'ShoppingAds' | 'DeliverySteps' | 'ReleaseCartAds';
		adId: number;
	}
	...
	async function fetchAd(conv: Conversation): Promise<Ad> {
		...
		if (conv.adType === 'ShoppingAds') {
			...
			// ReleaseCartAds
			const raw = await fetchFromAPI<any>(`/release-cart-ads/${conv.adId}`, {
				headers: { Authorization: `Bearer ${token}` }
			});
			return {
				title: raw.title,
	...
	// Quand l'utilisateur change la conversation via <select>
