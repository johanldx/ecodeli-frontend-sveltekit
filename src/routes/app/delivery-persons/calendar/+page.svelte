...
<div class="bg-base-100 min-h-screen p-6">
					...
					<tbody>
						{#each hours as hour}
							<tr class="hover:bg-base-100 transition">
								<td class="bg-base-100 text-right font-medium">{hour}</td>
								{#each days as day}
	...
	{#if showModal && selectedAppointment}
		<dialog class="modal modal-open">
			<div class="modal-box">
				<h3 class="text-lg font-bold">{selectedAppointment.title}</h3>
	...
	{#if showAddModal}
		<dialog class="modal modal-open">
			<div class="modal-box">
				<h3 class="mb-2 text-lg font-bold">Nouveau rendez-vous</h3>
				...
				<div class="modal-action flex flex-col gap-2">
					<button on:click={addAppointment} class="btn btn-success w-full"> Ajouter </button>
