<script lang="ts">
	...
	const CONTACT_LABEL = t('app.delivery.announce.contact');
	...
	async function loadData() {
	  ...
	  for (const ad of deliveryAds) {
		...
		for (const s of rawSteps) {
		  // Fetch full Location objects for each step
	...
	$: filteredShopping = shoppingAds.filter(ad =>
	...
	function handleContactShopping(id: number) {
	  console.log('Contact shopping', id);
	}
  
	function handleContactStep(id: number) {
	  console.log('Contact step', id);
	}
  </script>
	  ...
	  {#each filteredShopping as ad}
		<div class="card bg-base-100 mx-auto w-full shadow-sm">
		  {#if ad.imageUrls?.length > 0}
			<div class="carousel h-40 w-full rounded-t-lg sm:h-48">
			  {#each ad.imageUrls as url,i}
				<div id={`item-shop-${ad.id}-${i}`} class="carousel-item relative w-full">
				  <img src={url} alt={`Image ${i+1}`} class="h-full w-full rounded-t-lg object-cover" loading="lazy" />
				  <div class="bg-opacity-50 absolute right-2 bottom-2 rounded bg-black px-2 py-1 text-xs text-white">
			...
			<div class="mt-2 flex justify-center gap-2">
			  {#each ad.imageUrls as _,i}
				<button class="btn btn-xs" on:click={() => document.getElementById(`item-shop-${ad.id}-${i}`)?.scrollIntoView({behavior:'smooth',inline:'start'})}>
				  {i+1}
				</button>
			  {/each}
			</div>
		  ...
		  <div class="card-body p-4 md:p-6">
			...
			<p class="mt-1 text-xs"><strong>Date:</strong> {ad.deliveryDate ? new Date(ad.deliveryDate).toLocaleDateString() : '—'}</p>
			<p class="mt-1 text-xs"><strong>Départ:</strong> {ad.departureLocation.name}, {ad.departureLocation.city}</p>
			<p class="text-xs"><strong>Arrivée:</strong> {ad.arrivalLocation.name}, {ad.arrivalLocation.city}</p>
			...
			{/if}
			<button class="btn btn-primary btn-sm mt-4" on:click={() => handleContactShopping(ad.id)}>{CONTACT_LABEL}</button>
		  </div>
		</div>
	  {/each}
	  ...
	  {#each filteredSteps as step}
		<div class="card bg-base-100 mx-auto w-full shadow-sm">
		  {#if step.imageUrls?.length > 0}
			...
			<div class="mt-2 flex justify-center gap-2">
			  {#each step.imageUrls as _,i}
				<button class="btn btn-xs" on:click={() => document.getElementById(`item-step-${step.id}-{i}`)?.scrollIntoView({behavior:'smooth',inline:'start'})}>
				  {i+1}
				</button>
			  {/each}
			</div>
		  ...
		  <div class="card-body p-4 md:p-6">
			...
			<p class="mt-1 text-xs"><strong>Date:</strong> {step.deliveryDate ? new Date(step.deliveryDate).toLocaleDateString() : '—'}</p>
			<p class="mt-1 text-xs"><strong>Étape {step.stepNumber}:</strong> {step.departureLocation.name}, {step.departureLocation.city} → {step.arrivalLocation.name}, {step.arrivalLocation.city}</p>
			<button class="btn btn-primary btn-sm mt-4" on:click={() => handleContactStep(step.id)}>{CONTACT_LABEL}</button>
		  </div>
		</div>
	  {/each}
